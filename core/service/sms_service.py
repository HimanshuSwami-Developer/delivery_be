...
class SMSService:
    """
    Sends the OTP via 2Factor's SMS OTP API. Order-invoice notifications
    have no real-send backend currently — only "console" (logs/prints).
    ...
    (2Factor isn't an option here — its API only sends a code through its
    own pre-approved OTP template, not arbitrary text like an invoice.)
    """
